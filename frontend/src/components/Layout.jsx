...
export const Navigation = () => {
  ...
  return <nav className={`navigation ${scrolled ? 'nav-scrolled' : ''}`} aria-label="Main navigation" data-testid="main-navigation">
    <div className="nav-inner"><Link to="/" className="brand" aria-label="Appetiser India home" data-testid="nav-brand"><Wordmark id="nav-wordmark" /></Link>
      ...
      <div className="nav-right"><Action id="nav-start-project" variant="nav" arrow="up">Start a project</Action><button className="menu-toggle" data-testid="mobile-menu-toggle" aria-label={open ? 'Close menu' : 'Open menu'} aria-expanded={open} aria-controls="mobile-menu" onClick={() => setOpen(!open)}>{open ? <X /> : <Menu />}</button></div>
    </div>
...
export const ScrollManager = () => {
  ...
  useEffect(() => {
    if (reduced) return;
    const lenis = new Lenis({ autoRaf: true, duration: 1.15, smoothWheel: true, anchors: { offset: -100 } });
